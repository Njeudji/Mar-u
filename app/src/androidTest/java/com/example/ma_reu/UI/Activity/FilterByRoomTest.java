...
public class FilterByRoomTest {
    ...
    public void filterByRoomTest() {
        ...
        ViewInteraction chip = onView(
                allOf(withText("DIOP"),
                        childAtPosition(
                                allOf(withId(R.id.chipGroup_Room_Item),
                                        childAtPosition(
                                                withId(R.id.horizontalFilter),
                                                0)),
                                2)));
        chip.perform(scrollTo(), click());

        // Test effectué, ajout d'une réunion + filtre par salle et vérification que la réunion a bien été filtrer.
        onView(AllOf.allOf(withId(R.id.recycler_view_list_meetings)))
